...
static std::vector<uint8_t> crcCodeTest = {
...
static std::vector<uint8_t> crcCodeFull = {
...
TEST(InstructionParsingTest1, ParsingInstruction)
{
	std::string bitStream = utils::convertToBitStream(crcCodeTest);
	EXPECT_EQ(bitStream, crcBitStreamTest);
...
{
	std::string bitStream = utils::convertToBitStream(crcCodeFull);
	EVMDisasm disasm(bitStream);
...
{
	int64_t var;
	std::string bitStream1 = "1001110001000000000000000000000000000000000000000000000000000000";
	utils::bitStreamToVar<int64_t>(bitStream1, var);
	EXPECT_EQ(var, static_cast<int64_t>(0x239));
	std::string bitStream2 = "10110101011110111111011101111101"; // 0xbeefdead
	uint32_t var2;
	utils::bitStreamToVar<uint32_t>(bitStream2, var2);
	EXPECT_EQ(var2, static_cast<int32_t>(0xbeefdead));

	int64_t var3;
	std::string bitStream3 = "1110000000000000000000000000000000000000000000000000000000000111";
	utils::bitStreamToVar<int64_t>(bitStream3, var3);
	EXPECT_EQ(var3, static_cast<int64_t>(0xE000000000000007));
}
